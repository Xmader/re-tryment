...
function get_eval(text)
	...
	elseif head == 's' then
		ret = 0
		local os = game.os
			if name == "scene" and getExtra() then ret = 1
		elseif name == "trial" and getTrial() then ret = 1
		elseif name == "ps" and game.ps then ret = 1
		elseif name == "cs" and game.cs then ret = 1
...
tags["タイトル"] = function(e, p)
	systemreset = true
	if p.mode == "extra" then titleextra = true end
	if p["0"] == 'quick' then
		e:tag{"reset"}
	else
...
function tags.gotitle(e, p)
	systemreset = true
	if p and p.mode == "extra" then titleextra = true end
	e:tag{"call", file="system/ui.asb", label="go_title"}
	return 1
end
----------------------------------------
...
function exskip_end(e, p)
	if flg.exskip == 2 then
		...
		systemsound("selload")		-- sound再開
		uimask_off()				-- mask消去
		uitrans()
		flg.exskip = nil

		-- bgm
		local v = scr.bgm and scr.bgm.vol or 100
		bgm_fade{ vol=(v), time=(250) }
	...
	local sw = {
		...
		start = function()
			...
			if c > 0 and c <= 20 then
				...
				e:tag{"call", file="system/script.asb", label=(label)}
			else
...
function estag_call()
	...
	if s and m > 0 then
		...
		-- 実行
		local ng = { jump=1, call=1, trans=1, lydel=1 }
		local tg = v[1]
