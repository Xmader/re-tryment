...
----------------------------------------
-- system effect
function getSystemEffect()
	return conf.effect == 1	-- conf.sysani == 1
end
----------------------------------------
-- trans
----------------------------------------
...
function trans_main(p)
	...
	if sys then
		if not getSystemEffect() then time = 0 end
	...
	if time == 0 or not r then
		...
		eqtag{"trans", type="2", input=(input), time=(time), rule=(game.path.rule..rule..game.ruleext), vague=(vague)}
	end
end
----------------------------------------
-- trans終了
...
function uitrans(p)
	...
	p.fade = not getSystemEffect() and 0 or p.fade or p["0"] or init.ui_fade
	p.sys  = p.sys or true
	trans(p)
end
----------------------------------------
...
function flip2() e:enqueueTag{"flip"} end
----------------------------------------
-- lyc2
...
function systween(p)
	p.sys = true
	if not getSystemEffect() then p.time = 0 end
	tween(p)
end
----------------------------------------
function systween2(p)
	if getSystemEffect() then
		p.sys = true
		tween(p)
	end
end
